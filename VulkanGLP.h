...
struct VkResource {
    ...
    VkResource(VkGraphicsQueue& queue, VkFormat imageFormat, VkImageAspectFlagBits imageFlag, VkImageUsageFlags usageFlags) {
        this->VkQueue = &queue;
        queue.createImage(1, imageFormat,
            VK_IMAGE_TILING_OPTIMAL,
            usageFlags,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
            Image, ImageMemory);
        createImageView(queue, imageFormat, imageFlag, 1);
    }
    ~VkResource() {
...
private:
    ...
    void createImageView(VkGraphicsQueue& queue, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
        ...
        if (vkCreateImageView(queue.device, &viewInfo, nullptr, &ImageView) != VK_SUCCESS) {
