...
namespace vk {
    ...
    /*-----------------------------------------------------------------*/
    Shader_::Shader_(std::string const& filename, VkShaderStageFlagBits stage) : shaderStage(stage) {
        //checkLog(filename);
        auto code = readFile(".\\shaders\\" + filename + ".spv");
        createModule(code, shaderModule);
    }
    ...
    void Shader_::createModule(const std::vector<char>& code, VkShaderModule& shaderModule) {
        VkShaderModuleCreateInfo createInfo
        { VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO };
        createInfo.codeSize = code.size();
        createInfo.pCode = reinterpret_cast<const uint32_t*>(code.data());
    ...
    std::vector<char> Shader_::readFile(const std::string& filename) {
        std::ifstream file(filename, std::ios::ate | std::ios::binary);
